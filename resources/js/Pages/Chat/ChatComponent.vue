  ...
  <script setup>
   ...
   const sendTypingEvent = () => {
    ...
    onBeforeUnmount(() => {
        ...
        window.Echo.leave(`presence.chat`,user => {
            if (user.id === props.user.id) isUserOnline.value = false;
            // console.log("leave",isUserOnline.value);
    ...
    onMounted(() => {
        ...
        window.Echo.join(`presence.chat`)
            .here(users => {
                ...
                // console.log("here",isUserOnline.value);

            })
            .joining(user => {
                if (user.id === props.user.id) isUserOnline.value = true;
                // console.log("joining",isUserOnline.value);
                
            })
            .leaving(user => {
                if (user.id === props.user.id) isUserOnline.value = false;
                // console.log("leaving",isUserOnline.value);
